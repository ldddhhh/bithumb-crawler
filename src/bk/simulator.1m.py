...
def buyOrNot( fast_ks, slow_ks, slow_ds, rsis ):
	...
	stoch_using_time_base = '3m' # 스토캐스틱 지표에서 사용할 분봉 단위
	...
	using_rsis = rsis[ '5m' ][ 15 ]

	if buy_or_not:
		print( '* fast ks: {0}'.format( fast_ks[ stoch_using_time_base ][ stoch_using_period ] ) )
		print( '* slow ds: {0}'.format( slow_ds[ stoch_using_time_base ][ stoch_using_period ] ) )
		print( '* fast_k_inc_val: {0}'.format( fast_k_inc_val ) )

	if len( using_slow_ks ) > 3 and len( using_slow_ds ) > 3:
...
def sellOrNot( fast_ks, slow_ks, slow_ds, rsis ):
	...
	if slow_k_inc_val[ 1 ] <= 0: # fast_K_inc_val[ 1 ] < 매직넘버
		...
		sell_or_not = True
	elif using_slow_ks[ -1 ] >= 70:
		sell_or_not = True
	"""
	if fast_k_inc_val[ 1 ] < -20: # fast_K_inc_val[ 1 ] < 매직넘버
		sell_or_not = True
	elif using_fast_ks[ -1 ] <= using_slow_ds[ -1 ]-5:
		sell_or_not = True
...
def main():
	...
	for time_base in time_idx.keys():
		...
		for stoch_period in stoch_periods:
			...
			rsis[ time_base ][ stoch_period ]    = [ 0.0 ]
	...
	for data_fname in data_fnames:
		with open( data_fpath+data_fname, 'r', encoding='utf-8' ) as fr:
			for line_idx, line in enumerate( fr ):
				...
				if len( candle_datas[ '10m' ] ) >= stoch_periods[ -1 ]:
					for time_base in time_idx.keys():
						for stoch_period in stoch_periods:
							...
							rsi = getRSI( candle_datas[ time_base ], stoch_period )
							...
							if time_updated[ time_base ]:
								...
								if len( rsis[ time_base ][ stoch_period ] ) > 4:
									rsis[ time_base ][ stoch_period ] = rsis[ time_base ][ stoch_period ][ 1: ]
								rsis[ time_base ][ stoch_period ].append( rsi )
							else:
								fast_ks[ time_base ][ stoch_period ][ -1 ] = fast_k
								slow_ks[ time_base ][ stoch_period ][ -1 ] = slow_k
								slow_ds[ time_base ][ stoch_period ][ -1 ] = slow_d
								rsis[ time_base ][ stoch_period ][ -1 ]    = rsi



				if time_updated[ '1m' ]:
					if not has_bought:
						buy_or_not = buyOrNot( fast_ks, slow_ks, slow_ds, rsis )
						if buy_or_not:
						...
						sell_or_not = sellOrNot( fast_ks, slow_ks, slow_ds, rsis )
						if sell_or_not:
				...
				if len( candle_datas[ '10m' ] ) >= stoch_periods[ -1 ] and time_updated[ '5m' ]:
					...
					fw.write( '{0}\t{1}\t{2}\t{3}\t{4}\n'.format( out_start_price, out_end_price, out_fk, out_sk, out_sd ) )
