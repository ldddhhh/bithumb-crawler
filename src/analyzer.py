...
def buyOrNot( fast_ks, slow_ds, rsis ):
	'''
	현재 살지 지켜볼지 결정
	'''
	buy_or_not = False
...
def sellOrNot( fast_ks, slow_ds, rsis ):
	sell_or_not = False
	...
	for data_fname in data_fnames:
		with open( data_fpath+data_fname, 'r', encoding='utf-8' ) as fr:
			for line_idx, line in enumerate( fr ):
				...
				if not has_bought:
					buy_or_not = buyOrNot( fast_ks, slow_ds, rsis )
					if buy_or_not:
					...
					sell_or_not = sellOrNot( fast_ks, slow_ds, rsis )
					if sell_or_not:
